...
cdef class FastTextModelWrapper:
    ...
    # We can't access directly Dictionary in python because
    ...
    def classifier_predict_raw(self, text, k, label_prefix, encoding):
        ...
        for result in raw_results:
            ...
            label = label.replace(label_prefix, '')
            cpp_str = result[1]
            weight = float(cpp_str)
            labels.append(label)
            weights.append(weight)
        return list(zip(labels, weights))
    ...
    @property
    def saveSoftmax(self):
        return self.fm.saveSoftmax
...
def load_model(filename, save_softmax=0, label_prefix='', encoding='utf-8'):
    ...
    if model_name == 'skipgram' or model_name == 'cbow':
        ...
        for i in xrange(model.dict_nwords()):
            ...
            words.append(word)
        return WordVectorModel(model, words, save_softmax, encoding)
        ...
        return SupervisedModel(model, labels, label_prefix, save_softmax, encoding)
    else:
...
def train_wrapper(model_name, input_file, output, label_prefix, lr, dim, ws,
    ...
    py_args = [b'-input', b'-output', b'-lr', b'-dim', b'-ws', b'-epoch',
            ...
            b'-minn', b'-maxn', b'-thread', b'-lrUpdateRate', b'-t', b'-saveSoftmax']
    values = [input_file, output, lr, dim, ws, epoch, min_count, neg,
            word_ngrams, loss, bucket, minn, maxn, thread, lr_update_rate, t, save_softmax]
    ...
    model = load_model(output_bin, save_softmax, label_prefix, encoding=encoding)
...
def skipgram(input_file, output, lr=0.05, dim=100, ws=5, epoch=5, min_count=5,
        ...
        thread=12, lr_update_rate=100, t=1e-4, save_softmax=0, silent=1, encoding='utf-8'):
    label_prefix = ''
    pretrained_vectors = ''
    return train_wrapper('skipgram', input_file, output, label_prefix, lr,
            dim, ws, epoch, min_count, neg, word_ngrams, loss, bucket, minn,
            maxn, thread, lr_update_rate, t, save_softmax, pretrained_vectors, silent, encoding='utf-8')
...
def cbow(input_file, output, lr=0.05, dim=100, ws=5, epoch=5, min_count=5,
    ...
    return train_wrapper('cbow', input_file, output, label_prefix, lr, dim,
            ...
            thread, lr_update_rate, t, save_softmax, pretrained_vectors, silent, encoding)
...
def supervised(input_file, output, label_prefix='__label__', lr=0.1, dim=100,
        ...
        t=1e-4, save_softmax=0, pretrained_vectors='', silent=1, encoding='utf-8'):
    return train_wrapper('supervised', input_file, output, label_prefix, lr,
            dim, ws, epoch, min_count, neg, word_ngrams, loss, bucket, minn,
            maxn, thread, lr_update_rate, t, save_softmax, pretrained_vectors, silent, encoding)
